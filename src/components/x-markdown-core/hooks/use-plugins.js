 ...
 * @FilePath     : /bi-agents/src/components/x-markdown-core/hooks/use-plugins.js
 * @Description  :
 *
 */
...
function usePlugins(props) {
  ...
  const remark = computed(() => {
    ...
    const mathPlugins = enableLatex.value
      ? [remarkMathExtended, remarkMath]
      : [];

    const base = [...mathPlugins, enableBreaks.value && remarkBreaks].filter(
      Boolean,
    );
