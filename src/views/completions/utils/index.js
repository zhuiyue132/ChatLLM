...
export const getProviderImageUrl = (name) => {
  return new URL(
    `../../../assets/images/ai-provider/${name}.png`,
    import.meta.url,
  ).href;
};
...
export const getProviderImageByModelName = (modelName) => {
  ...
  const providerName = modelName.match(
    /claude|deepseek|qwen|doubao|gemini|llama|gpt/gi,
  )?.[0];
  ...
  items.forEach((item) => {
    ...
    if (item.parentId === null || item.parentId === undefined) {
      ...
      if (parent) {
        ...
        // 如果找不到父节点，作为顶级节点处理
        console.warn(
          `找不到parentId为${item.parentId}的父节点，将id为${item.id}的节点作为顶级节点`,
        );
        roots.push(node);
      }
    }
...
export const buildList = (items) => {
  ...
  const result = userMessages.map((userMsg) => {
    ...
    return {
      ...
      rightCurrentIndex:
        rightChildren.length > 0 ? rightChildren.length - 1 : 0,
